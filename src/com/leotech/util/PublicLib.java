...
//返回非空的byte位置
	...
	public static int getInt(InputStream inStream) {
		try {
			return (int) ((inStream.read()&0xFF)   
		            | ((inStream.read()<<8) & 0xFF00)  
		            | ((inStream.read()<<16)& 0xFF0000)   
		            | ((inStream.read()<<24) & 0xFF000000));  
...
//					| ((inStream.read()) << 16) | ((inStream.read()) << 24);
	...
	//获得double型变量函数
	public static double getDouble(InputStream inStream) {
		double javadouble;
		try {
			javadouble = (double) (inStream.read() + ((inStream.read()) << 8)
					+ ((inStream.read()) << 16) + ((inStream.read()) << 24)
					+ ((inStream.read()) << 32) + ((inStream.read()) << 40)
					+ ((inStream.read()) << 48) + ((inStream.read()) << 56));
		} catch (IOException e) {
	...
	public static String getDateByInt(int sec){
		String dateTime = "";
		long millisecond = sec * 1000l;
